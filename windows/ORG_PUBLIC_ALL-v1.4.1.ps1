...
# ===========================
# Profile cleanup after 8 days
# ===========================

# Group Policy path
$CleanupPolicyPath = "HKLM:\SOFTWARE\Policies\Microsoft\Windows\System"

if (-not (Test-Path $CleanupPolicyPath)) {
    New-Item -Path $CleanupPolicyPath -Force | Out-Null
}

# Set CleanupProfiles (days before deletion)
New-ItemProperty -Path $CleanupPolicyPath `
    -Name "CleanupProfiles" -PropertyType DWord -Value 8 -Force | Out-Null

# ===========================
# Idle logoff task (20 minutes, ONIDLE trigger)
# ===========================

$taskName = "Idle 20m Logoff"
...
for ($i = 1; $i -lt $output.Count; $i++) {
    ...
    # Logoff if idle threshold reached, or session is disconnected past threshold.
...
foreach ($oldName in @("AutoLogoff-RealIdle", "Idle 2h Logoff", $taskName)) {
    try {
...
# Create scheduled task that checks every minute and logs off sessions idle >= 20 minutes
$logoffCmd  = "powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File `"$logoffScriptPath`" -Minutes 20"
...
Write-Host "Scheduled task created: $taskName (logs off all active user sessions after 20 minutes idle)" -ForegroundColor Green

# ===========================
# Display power policy (screen off after 10 minutes, sleep after 1 hour) and lock it
# ===========================
